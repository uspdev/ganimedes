...
public class RelatorioEstagio {
	...
	public void gerarDocumento(List<Estagio> estagios, Date di, Date df) throws DocumentException, IOException {
		...
		try {
			...
			pdf = montarRelatorio(estagios, di, df, pdf);
	...
	public Document montarRelatorio(List<Estagio> estagios, Date di, Date df, Document pdf) throws DocumentException {
		...
		t.addCell(getCellTitle("Estágios no período de " + DataUtility.dateToStringDate(di) + " até " + DataUtility.dateToStringDate(df), fb, true, 3));
		...
		for (Estagio e : estagios) {
			try {
				t.addCell(getCell("\n" + e.getAluno().getNompes() + Chunk.NEWLINE + e.getAluno().getCodpes(), fn, false, 1));
